...
struct emerald_dragonAI : public WorldBossAI
{
    ...
    void Reset() override
    {
        ...
        events.ScheduleEvent(EVENT_TAIL_SWEEP, Seconds(4));
        events.ScheduleEvent(EVENT_NOXIOUS_BREATH, randtime(Seconds(7) + Milliseconds(500), Seconds(15)));
        events.ScheduleEvent(EVENT_SEEPING_FOG, randtime(Seconds(12) + Milliseconds(500), Seconds(20)));
    }
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            case EVENT_SEEPING_FOG:
                ...
                DoCastSelf(SPELL_SEEPING_FOG_LEFT, true);
                DoCastSelf(SPELL_SEEPING_FOG_RIGHT, true);
                events.Repeat(randtime(Minutes(2), Minutes(2) + Seconds(30)));
                break;
            case EVENT_NOXIOUS_BREATH:
                // Noxious Breath is cast on random intervals, no less than 7.5 seconds between
                DoCastAOE(SPELL_NOXIOUS_BREATH);
                events.Repeat(randtime(Seconds(7) + Milliseconds(500), Seconds(15)));
                break;
    ...
    public:
        ...
        struct boss_ysondreAI : public emerald_dragonAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(EVENT_LIGHTNING_WAVE, Seconds(12));
            }
            ...
            {
                if (!HealthAbovePct(100 - 25 * stage))
                {
    ...
    public:
        ...
        struct boss_lethonAI : public emerald_dragonAI
        {
            ...
            void Reset() override
            {
                stage = 1;
                emerald_dragonAI::Reset();
                events.ScheduleEvent(EVENT_SHADOW_BOLT_WHIRL, Seconds(10));
            }
            ...
            void ExecuteEvent(uint32 eventId) override
            {
                switch (eventId)
                {
                    case EVENT_SHADOW_BOLT_WHIRL:
                        DoCastAOE(SPELL_SHADOW_BOLT_WHIRL);
...
class npc_spirit_shade : public CreatureScript
{
    public:
        ...
        {
            npc_spirit_shadeAI(Creature* creature) : PassiveAI(creature), summonerGuid() { }
            ...
            {
                if (moveType == FOLLOW_MOTION_TYPE && data == summonerGuid.GetCounter())
                {
                    DoCastAOE(SPELL_DARK_OFFERING);
    ...
    public:
        ...
        struct boss_emerissAI : public emerald_dragonAI
        {
            ...
            void Reset() override
            {
                stage = 1;
                emerald_dragonAI::Reset();
                events.ScheduleEvent(EVENT_VOLATILE_INFECTION, Seconds(12));
            }
            ...
            void DamageTaken(Unit* /*attacker*/, uint32& /*damage*/) override
            {
                if (!HealthAbovePct(100 - 25 * stage))
                {
                    ...
                    DoCastAOE(SPELL_CORRUPTION_OF_EARTH, true);
                    ++stage;
                }
            }
    ...
    public:
        ...
        struct boss_taerarAI : public emerald_dragonAI
        {
            ...
            void Reset() override
            {
                ...
                emerald_dragonAI::Reset();
                events.ScheduleEvent(EVENT_ARCANE_BLAST, Seconds(12));
                events.ScheduleEvent(EVENT_BELLOWING_ROAR, Seconds(30));
            }
            ...
            void DamageTaken(Unit* /*attacker*/, uint32& /*damage*/) override
            {
                ...
                // Note: stage holds the amount of times they have been summoned
                if (!banished && !HealthAbovePct(100 - 25 * stage))
                {
                    banished = true;
                    banishedTimer = 60000;
            ...
            void ExecuteEvent(uint32 eventId) override
            {
                switch (eventId)
                {
                    case EVENT_ARCANE_BLAST:
                        ...
                        events.Repeat(randtime(Seconds(7), Seconds(12)));
                        break;
        ...
        private:
            bool   banished;                              // used for shades activation testing
            uint32 banishedTimer;                         // counter for banishment timeout
            uint8  shades;                                // keep track of how many shades are dead
            uint8  stage;                                 // check which "shade phase" we're at (75-50-25 percentage counters)
        };
