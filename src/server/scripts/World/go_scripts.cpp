...
enum BarovJournal
{
    SPELL_CREATE_FELCLOTH_BAG   = 26086,
    SPELL_FELCLOTH_BAG          = 26095
};
...
class go_barov_journal : public GameObjectScript
{
    public:
        ...
        struct go_barov_journalAI : public GameObjectAI
        {
            ...
            bool GossipHello(Player* player) override
            {
                if (player->HasSkill(SKILL_TAILORING) && player->GetBaseSkillValue(SKILL_TAILORING) >= 280 && !player->HasSpell(SPELL_CREATE_FELCLOTH_BAG))
                    player->CastSpell(player, SPELL_FELCLOTH_BAG, false);
...
enum OrbCommand
{
    SPELL_TELE_BLACKWING_LAIR   = 23460
};
        ...
        struct go_orb_of_commandAI : public GameObjectAI
        {
            ...
            bool GossipHello(Player* player) override
            {
                if (player->GetQuestRewardStatus(7761))
                    player->CastSpell(player, SPELL_TELE_BLACKWING_LAIR, true);
...
enum TabletMadness
{
    SPELL_CREATE_MOJO_MADNESS   = 24266,
    SPELL_LEARN_MOJO_MADNESS    = 24267
};
...
class go_tablet_of_madness : public GameObjectScript
{
    public:
        ...
        struct go_tablet_of_madnessAI : public GameObjectAI
        {
            ...
            bool GossipHello(Player* player) override
            {
                if (player->HasSkill(SKILL_ALCHEMY) && player->GetSkillValue(SKILL_ALCHEMY) >= 300 && !player->HasSpell(SPELL_CREATE_MOJO_MADNESS))
                    player->CastSpell(player, SPELL_LEARN_MOJO_MADNESS, false);
        ...
        struct go_tablet_of_the_sevenAI : public GameObjectAI
        {
            ...
            bool GossipHello(Player* player) override
            {
                ...
                if (player->GetQuestStatus(4296) == QUEST_STATUS_INCOMPLETE)
                    player->CastSpell(player, SPELL_CREATE_TRANSCRIPT, false);
