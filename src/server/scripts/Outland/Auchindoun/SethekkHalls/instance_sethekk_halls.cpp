...
class instance_sethekk_halls : public InstanceMapScript
{
    public:
        ...
        struct instance_sethekk_halls_InstanceMapScript : public InstanceScript
        {
            ...
            {
                if (eventId == EVENT_SUMMON_ANZU)
                {
                    Position const anzuPos = { -78.360298f, 288.524994f, 26.483166f, 3.213590f };
                    if (!SummonedAnzu && GetBossState(DATA_ANZU) != DONE)
                    {
                        SummonedAnzu = true;
                        instance->SummonCreature(NPC_ANZU, anzuPos);
                    }
                }
            }
