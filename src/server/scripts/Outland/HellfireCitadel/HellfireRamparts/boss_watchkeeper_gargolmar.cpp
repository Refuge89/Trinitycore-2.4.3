...
class boss_watchkeeper_gargolmar : public CreatureScript
{
    public:
        ...
        struct boss_watchkeeper_gargolmarAI : public BossAI
        {
            ...
            {
                Talk(SAY_AGGRO);
                events.ScheduleEvent(EVENT_MORTAL_WOUND, Seconds(5));
                events.ScheduleEvent(EVENT_SURGE, Seconds(4));
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_MORTAL_WOUND:
                            ...
                            events.Repeat(randtime(Seconds(5), Seconds(13)));
                            break;
                        case EVENT_SURGE:
                            Talk(SAY_SURGE);
                            if (Unit* target = SelectTarget(SELECT_TARGET_MAXDISTANCE, 0, 40.0f))
                ...
                if (!retaliation && HealthBelowPct(20))
                {
                    events.ScheduleEvent(EVENT_RETALIATION, Seconds(1));
                    retaliation = true;
                }

                if (!yelledForHeal && HealthBelowPct(40))
                {
                    Talk(SAY_HEAL);
                    yelledForHeal = true;
                }
