...
class spell_item_dementia : public AuraScript
{
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_item_dementia::HandlePeriodicDummy, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
...
class spell_item_mind_control_cap : public SpellScript
{
    ...
    bool Validate(SpellInfo const* /*spell*/) override
    {
        return ValidateSpellInfo({ SPELL_GNOMISH_MIND_CONTROL_CAP });
    }
    ...
    void HandleDummy(SpellEffIndex /* effIndex */)
    {
        ...
        if (Unit* target = GetHitUnit())
        {
            if (roll_chance_i(ROLL_CHANCE_NO_BACKFIRE))
                caster->CastSpell(target, SPELL_GNOMISH_MIND_CONTROL_CAP, GetCastItem());
            else
