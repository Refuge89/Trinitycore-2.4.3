...
enum Spells
{
    ...
    SPELL_TERRIFYING_ROAR               = 29685, // AOE Fear
    ...
    public:
        ...
        struct boss_gluthAI : public BossAI
        {
            ...
            void JustEngagedWith(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_DECIMATE, randtime(Minutes(1) + Seconds(40), Minutes(1) + Seconds(50)));
                events.ScheduleEvent(EVENT_BERSERK, Minutes(5) + Seconds(30));
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_DECIMATE:
                            ...
                            events.Repeat(randtime(Minutes(1) + Seconds(40), Minutes(1) + Seconds(50)));
                            break;
                        case EVENT_BERSERK:
                            ...
                            break;
                        case EVENT_SUMMON:
                            me->SummonCreatureGroup(SUMMON_GROUP_CHOW);
                            events.Repeat(Seconds(10));
                            break;
                        case EVENT_SEARCH_ZOMBIE_SINGLE:
                        {
                            ...
                            for (SummonList::const_iterator itr = summons.begin(); !zombie && itr != summons.end(); ++itr)
                            {
                                ...
                                if (!zombie || !zombie->IsAlive() || !zombie->IsWithinDistInMap(me, 5.0))
                                    zombie = nullptr;
                            }
                        ...
                        case EVENT_KILL_ZOMBIE_SINGLE:
                        {
                            ...
                            if (zombieToBeEaten && zombieToBeEaten->IsAlive() && zombieToBeEaten->IsWithinDistInMap(me, 5.0))
                        ...
                        case EVENT_SEARCH_ZOMBIE_MULTI:
                        {
                            ...
                            {
                                zombie = ObjectAccessor::GetCreature(*me, *itr);
                                if (zombie && zombie->IsAlive() && zombie->GetExactDist2d(me) > 5.0)
                                    zombie = nullptr;
                            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (state == STATE_ZOMBIE_DECIMATED)
                {
                    ...
                    if (Creature* gluth = ObjectAccessor::GetCreature(*me, GluthGUID))
                    {
                        ...
                        if (timer > 1600 && me->GetExactDist2d(gluth) > 5.0f && me->CanFreeMove()) // it takes about 1600 ms for the animation to cycle. This way, the animation looks relatively smooth.
                        {
                        ...
                        if (me->GetExactDist2d(gluth) <= 5.0f)
                            me->StopMoving();
                    }
                }
