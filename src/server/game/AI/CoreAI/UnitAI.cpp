...
bool UnitAI::DoSpellAttackIfReady(uint32 spell)
{
    ...
    {
        if (me->IsWithinCombatRange(me->GetVictim(), spellInfo->GetMaxRange()))
        {
...
float UnitAI::DoGetSpellMaxRange(uint32 spellId)
{
    ...
    return spellInfo ? spellInfo->GetMaxRange() : 0;
}
    ...
    switch (AISpellInfo[spellId].target)
    {
        ...
        case AITARGET_ENEMY:
        {
            if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId))
            {
                ...
                target = SelectTarget(SELECT_TARGET_RANDOM, 0, spellInfo->GetMaxRange(), playerOnly);
            }
            break;
        }
        case AITARGET_ALLY:
