...
void Channel::Say(ObjectGuid guid, std::string const& what, uint32 lang) const
{
    ...
    auto builder = [&](WorldPacket& data, LocaleConstant locale)
    {
        ...
        if (Player* player = ObjectAccessor::FindConnectedPlayer(guid))
            ChatHandler::BuildChatPacket(data, CHAT_MSG_CHANNEL, Language(lang), player, player, what, GetName(localeIdx));
        else
            ChatHandler::BuildChatPacket(data, CHAT_MSG_CHANNEL, Language(lang), guid, guid, what, 0, "", "", false, GetName(localeIdx));
    };
