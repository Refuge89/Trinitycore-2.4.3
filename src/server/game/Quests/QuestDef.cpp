...
{
    if (player && _rewardBonusMoney && _level > 0)
    {
        float rewXp = _rewardBonusMoney / 0.6f;
        if (_level >= 65)
            rewXp = _rewardBonusMoney / 6.0f;
        else if (_level == 64)
            rewXp = _rewardBonusMoney / 4.8f;
        else if (_level == 63)
            rewXp = _rewardBonusMoney / 3.6f;
        else if (_level == 62)
            rewXp = _rewardBonusMoney / 2.4f;
        else if (_level == 61)
            rewXp = _rewardBonusMoney / 1.2f;

        uint8 pLevel = player->getLevel();
        uint32 xp = uint32(ceilf(rewXp * 0.1f));
        if (pLevel <= _level + 5)
            xp = uint32(ceilf(rewXp));
        else if (pLevel == _level + 6)
            xp = uint32(ceilf(rewXp * 0.8f));
        else if (pLevel == _level + 7)
            xp = uint32(ceilf(rewXp * 0.6f));
        else if (pLevel == _level + 8)
            xp = uint32(ceilf(rewXp * 0.4f));
        else if (pLevel == _level + 9)
            xp = uint32(ceilf(rewXp * 0.2f));

        if (sWorld->getIntConfig(CONFIG_MIN_QUEST_SCALED_XP_RATIO))
        {
            uint32 minScaledXP = uint32(ceilf(rewXp * sWorld->getIntConfig(CONFIG_MIN_QUEST_SCALED_XP_RATIO)));
            return std::max(minScaledXP, xp);
        }
...
bool Quest::IsRaidQuest() const
{
    if (_type == QUEST_TYPE_RAID)
        return true;
