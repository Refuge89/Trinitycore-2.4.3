        ...
        if (vmgr->isMapLoadingEnabled())
        {
            ...
            switch (result)
            {
                ...
                case VMAP::LoadResult::FileNotFound:
                    TC_LOG_ERROR("maps", "VMap .vmtree file '%s' does not exist", (sWorld->GetDataPath() + "vmaps/" + name).c_str());
                    ...
                    return false;
                case VMAP::LoadResult::FileNotFoundTile:
                    name = VMAP::StaticMapTree::getTileFileName(mapid, gx, gy);
                    TC_LOG_ERROR("maps", "VMap .vmtile file '%s' does not exist", (sWorld->GetDataPath() + "vmaps/" + name).c_str());
                    TC_LOG_ERROR("maps", "Please place VMAP files (*.vmtree and *.vmtile in the vmap directory (%s), or correct the DataDir setting in your worldserver.conf file.", (sWorld->GetDataPath() + "vmaps/").c_str());
                    return false;
