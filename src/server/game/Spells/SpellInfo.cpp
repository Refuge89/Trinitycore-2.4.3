...
SpellEffectInfo::SpellEffectInfo(SpellEntry const* spellEntry, SpellInfo const* spellInfo, uint8 effIndex)
{
    ...
    DicePointsPerLevel = spellEntry->EffectDicePerLevel[effIndex];
...
SpellCastResult SpellInfo::CheckTarget(WorldObject const* caster, WorldObject const* target, bool implicit /*= true*/) const
{
    ...
    {
        if (!(unitCaster->GetCharmerOrOwner() == target))
        {
...
float SpellInfo::GetMinRange() const
{
    if (!RangeEntry)
        return 0.0f;
...
float SpellInfo::GetMaxRange(WorldObject* caster /*= nullptr*/, Spell* spell /*= nullptr*/) const
{
    if (!RangeEntry)
        return 0.0f;
