...
SpellEffectHandlerFn SpellEffectHandlers[TOTAL_SPELL_EFFECTS] =
{
    ...
    &Spell::EffectNULL,                                     // 87 SPELL_EFFECT_GAMEOBJECT_DAMAGE
    &Spell::EffectNULL,                                     // 88 SPELL_EFFECT_GAMEOBJECT_REPAIR
    &Spell::EffectNULL,                                     // 89 SPELL_EFFECT_GAMEOBJECT_SET_DESTRUCTION_STATE
    ...
    &Spell::EffectUnused,                                   //154 SPELL_EFFECT_DISCOVER_TAXI
    &Spell::EffectUnused,                                   //155 SPELL_EFFECT_TITAN_GRIP Allows you to equip two-handed axes, maces and swords in one hand, but you attack $49152s1% slower than normal.
    &Spell::EffectUnused,                                   //156 SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC
    &Spell::EffectUnused,                                   //157 SPELL_EFFECT_CREATE_ITEM_2            create item or create item template and replace by some randon spell loot item
    &Spell::EffectUnused,                                   //158 SPELL_EFFECT_MILLING                  milling
    &Spell::EffectUnused,                                   //159 SPELL_EFFECT_ALLOW_RENAME_PET         allow rename pet once again
    &Spell::EffectUnused,                                   //160 SPELL_EFFECT_FORCE_CAST_2
    &Spell::EffectUnused,                                   //161 SPELL_EFFECT_TALENT_SPEC_COUNT        second talent spec (learn/revert)
    &Spell::EffectUnused,                                   //162 SPELL_EFFECT_TALENT_SPEC_SELECT       activate primary/secondary spec
    &Spell::EffectUnused,                                   //163 unused
    &Spell::EffectUnused,                                   //164 SPELL_EFFECT_REMOVE_AURA
};
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    else
    {
        ...
        float max_dis = m_spellInfo->GetMaxRange();
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const* properties, uint32 numGuardians)
{
    ...
    for (uint32 count = 0; count < numGuardians; ++count)
    {
        ...
        {
            if (uint32 weapon = unitCaster->GetUInt32Value(PLAYER_VISIBLE_ITEM_16_0))
            {
