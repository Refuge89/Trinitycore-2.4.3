...
{
    SetCritChance(CalcPeriodicCritChance(caster));
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            switch (GetSpellInfo()->Effects[i].ApplyAuraName)
            {
                case SPELL_AURA_PERIODIC_HEAL:
                    SetDonePct(caster->SpellHealingPctDone(GetUnitOwner(), GetSpellInfo()));
                    break;
                case SPELL_AURA_PERIODIC_DAMAGE:
                case SPELL_AURA_PERIODIC_LEECH:
                    SetDonePct(caster->SpellDamagePctDone(GetUnitOwner(), GetSpellInfo(), DOT));
                    break;
                default:
                    break;
            }
