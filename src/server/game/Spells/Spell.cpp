...
void Spell::SelectImplicitNearbyTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    switch (targetType.GetCheckType())
    {
        case TARGET_CHECK_ENEMY:
            range = m_spellInfo->GetMaxRange(m_caster, this);
            break;
...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (G3D::fuzzyEq(radius, 0.f))
        radius = m_spellInfo->GetMaxRange(m_caster, this);
...
void Spell::SelectImplicitCasterDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    switch (targetType.GetTarget())
    {
        ...
        case TARGET_DEST_CASTER_FISHING:
        {
            ...
            float maxDist = m_spellInfo->GetMaxRange();
...
void Spell::SelectImplicitTrajTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    float bestDist = m_spellInfo->GetMaxRange();
    if (SpellInfo const* triggerSpellInfo = sSpellMgr->GetSpellInfo(m_spellInfo->Effects[effIndex].TriggerSpell))
        bestDist = std::min(std::max(bestDist, triggerSpellInfo->GetMaxRange()), std::min(dist2d, 300.0f));
...
void Spell::prepareDataForTriggerSystem()
{
    ...
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER &&
        (m_spellInfo->SpellFamilyFlags[0] & 0x1C)) // Freezing and Frost Trap, Explosive and Immolation Trap
    {
        ...
        if (_spellHitTarget->GetTypeId() == TYPEID_UNIT)
        {
            if (_spellHitTarget->ToCreature()->IsAIEnabled)
            {
                if (spell->m_caster->GetTypeId() == TYPEID_GAMEOBJECT)
                    _spellHitTarget->ToCreature()->AI()->SpellHitByGo(spell->m_caster->ToGameObject(), spell->m_spellInfo);
                else
                    _spellHitTarget->ToCreature()->AI()->SpellHitByUnit(spell->m_caster->ToUnit(), spell->m_spellInfo);
            }
        }

        if (spell->m_caster->GetTypeId() == TYPEID_UNIT && spell->m_caster->ToCreature()->IsAIEnabled)
            spell->m_caster->ToCreature()->AI()->SpellHitUnitTarget(_spellHitTarget, spell->m_spellInfo);
        else if (spell->m_caster->GetTypeId() == TYPEID_GAMEOBJECT && spell->m_caster->ToGameObject()->AI())
            spell->m_caster->ToGameObject()->AI()->SpellHitUnitTarget(_spellHitTarget, spell->m_spellInfo);
...
void Spell::GOTargetInfo::DoTargetSpellHit(Spell* spell, uint8 effIndex)
{
    ...
    {
        if (spell->m_caster->GetTypeId() == TYPEID_GAMEOBJECT)
            go->AI()->SpellHitByGo(spell->m_caster->ToGameObject(), spell->m_spellInfo);
        else
            go->AI()->SpellHitByUnit(spell->m_caster->ToUnit(), spell->m_spellInfo);
    }

    if (spell->m_caster->GetTypeId() == TYPEID_UNIT && spell->m_caster->ToCreature()->IsAIEnabled)
        spell->m_caster->ToCreature()->AI()->SpellHitGoTarget(go, spell->m_spellInfo);
    else if (spell->m_caster->GetTypeId() == TYPEID_GAMEOBJECT && spell->m_caster->ToGameObject()->AI())
        spell->m_caster->ToGameObject()->AI()->SpellHitGoTarget(go, spell->m_spellInfo);
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                if (GetSpellInfo()->NeedsExplicitUnitTarget())
                {
                    ...
                    float range = m_spellInfo->GetMaxRange(unitCaster, this) * 1.5f + objSize; // can't be overly strict
...
std::pair<float, float> Spell::GetMinMaxRange(bool strict) const
{
    ...
    if (m_spellInfo->RangeEntry)
    {
        ...
        else
        {
            ...
            minRange = m_spellInfo->GetMinRange() + meleeRange;
            maxRange = m_spellInfo->GetMaxRange();
