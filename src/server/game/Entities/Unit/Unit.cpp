    ...
    m_threatManager(this), m_comboTarget(nullptr), m_comboPoints(0), m_spellHistory(new SpellHistory(this))
{
    ...
    _updateFlag = (UPDATEFLAG_HIGHGUID | UPDATEFLAG_LIVING | UPDATEFLAG_STATIONARY_POSITION);
...
bool Unit::HasAuraTypeWithFamilyFlags(AuraType auraType, uint32 familyName, uint32 familyFlags) const
{
    ...
    for (AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        if (SpellInfo const* iterSpellProto = (*itr)->GetSpellInfo())
            if (iterSpellProto->SpellFamilyName == familyName && iterSpellProto->SpellFamilyFlags.HasFlag(familyFlags, familyFlags))
                return true;
    return false;
}
...
/*static*/ uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask /*damageSchoolMask*/, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if (victim->GetTypeId() != TYPEID_PLAYER && !victim->IsControlledByPlayer())
    {
    ...
    else
    {
        ...
        {
            if (victim != attacker && (!spellProto || spellProto->HasAttribute(SPELL_ATTR3_TREAT_AS_PERIODIC)))
            {
                if (Spell* spell = victim->m_currentSpells[CURRENT_GENERIC_SPELL])
                {
                    if (spell->getState() == SPELL_STATE_PREPARING)
...
void Unit::_AddAura(UnitAura* aura, Unit* caster)
{
    ...
    {
        ASSERT((IsInWorld() && !IsDuringRemoveFromWorld()) || (aura->GetCasterGUID() == GetGUID()) || IsLoading());
            ...
            if (Unit* unitStealer = stealer->ToUnit())
            {
                ...
                else
                {
                    ...
                    if (Aura* newAura = Aura::TryRefreshStackOrCreate(createInfo))
                    {
                        ...
                        newAura->SetLoadedState(aura->GetMaxDuration(), int32(dur), aura->GetCharges(), 1, recalculateMask, aura->GetCritChance(), aura->CanApplyResilience(), &damage[0]);
...
void Unit::RemoveAurasWithFamily(SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, ObjectGuid casterGUID)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        Aura const* aura = iter->second->GetBase();
        if (!casterGUID || aura->GetCasterGUID() == casterGUID)
        {
            ...
            if (spell->SpellFamilyName == uint32(family) && spell->SpellFamilyFlags.HasFlag(familyFlag1, familyFlag2))
            {
                RemoveAura(iter);
                continue;
            }
...
AuraEffect* Unit::GetAuraEffectByFamilyFlags(AuraType type, SpellFamilyNames name, uint32 familyFlag1, uint32 familyFlag2, ObjectGuid casterGUID) const
{
    AuraEffectList const& auras = GetAuraEffectsByType(type);
    for (AuraEffectList::const_iterator i = auras.begin(); i != auras.end(); ++i)
    {
        SpellInfo const* spell = (*i)->GetSpellInfo();
        if (spell->SpellFamilyName == uint32(name) && spell->SpellFamilyFlags.HasFlag(familyFlag1, familyFlag2))
        {
...
{
    WorldPacket data(SMSG_SPELLNONMELEEDAMAGELOG, 8 + 8 + 4 + 4 + 1 + 4 + 4 + 1 + 1 + 4 + 4 + 1); // we guess size
...
float Unit::SpellDamagePctDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype) const
{
    ...
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            {
                if (victim->GetAuraEffectByFamilyFlags(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID, 0x200002, 0))
                    AddPct(DoneTotalMod, (*i)->GetAmount());
                break;
            }
            // Tundra Stalker
            ...
            case 7377:
            {
                if (victim->GetAuraEffectByFamilyFlags(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_PRIEST, 0x8000, 0, GetGUID()))
                    AddPct(DoneTotalMod, (*i)->GetAmount());
                break;
            }
            // Marked for Death
            ...
            case 7602:
            {
                if (victim->GetAuraEffectByFamilyFlags(SPELL_AURA_MOD_STALKED, SPELLFAMILY_HUNTER, 0x400, 0))
                    AddPct(DoneTotalMod, (*i)->GetAmount());
                break;
            }
            // Dirty Deeds
    ...
    switch (spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PALADIN:
            ...
            if ((spellProto->SpellFamilyFlags[1] & 0x8) && spellProto->SpellIconID == 2292)
            {
                // Get stack of Holy Vengeance
                uint32 stacks = 0;
                ...
                // + 10% for each application of Holy Vengeance
                if (stacks)
    ...
    switch (spellInfo->DmgClass)
    {
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (caster)
            {
                    ...
                    case SPELLFAMILY_DRUID:
                        ...
                        if (spellInfo->SpellFamilyFlags[0] & 0x4 && spellInfo->SpellIconID == 1485)
                        {
                            ...
                            if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_DRUID, 1771, 0))
                                if (GetAuraEffectByFamilyFlags(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID, 0x00000002, 0))
                                    crit_chance += aurEff->GetAmount();
                            break;
                        }
                        break;
                ...
                if (GetTypeId() == TYPEID_UNIT)
                {
                    int32 const levelDiff = static_cast<int32>(GetLevelForTarget(this)) - caster->getLevel();
...
void CharmInfo::SaveStayPosition()
{
    ...
    if (_unit->movespline->onTransport)
        if (Transport* transport = _unit->GetTransport())
...
void Unit::Whisper(std::string const& text, Language language, Player* target, bool isBossWhisper /*= false*/)
{
    ...
    WorldPacket data;
    ChatHandler::BuildChatPacket(data, isBossWhisper ? CHAT_MSG_RAID_BOSS_WHISPER : CHAT_MSG_MONSTER_WHISPER, language, this, target, text, "", locale);
...
void Unit::Whisper(uint32 textId, Player* target, bool isBossWhisper /*= false*/)
{
    ...
    LocaleConstant locale = target->GetSession()->GetSessionDbLocaleIndex();
    WorldPacket data;
    ChatHandler::BuildChatPacket(data, isBossWhisper ? CHAT_MSG_RAID_BOSS_WHISPER : CHAT_MSG_MONSTER_WHISPER, LANG_UNIVERSAL, this, target, bct->GetText(locale, getGender()), "", locale);
    target->SendDirectMessage(&data);
}
