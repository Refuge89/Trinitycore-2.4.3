...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current)
{
    ...
    switch (getPetType())
    {
        ...
        case HUNTER_PET:
            ...
            SetByteFlag(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PVP_FLAG, UNIT_BYTE2_FLAG_SANCTUARY | UNIT_BYTE2_FLAG_AURAS | UNIT_BYTE2_FLAG_UNK5);
    ...
    if (summonSpellId)
    {
        WorldPacket data(SMSG_SPELL_GO, 8 + 8 + 4 + 2 + 4);
        data << owner->GetPackGUID();
        data << owner->GetPackGUID();
        data << uint32(summonSpellId);
        data << uint16(256); // CAST_FLAG_UNKNOWN3
        data << uint32(0);
