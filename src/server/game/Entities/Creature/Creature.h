...
#define MAX_VENDOR_ITEMS 255                                // Limitation in item count in SMSG_LIST_INVENTORY
...
class TC_GAME_API Creature : public Unit, public GridObject<Creature>, public MapObject
{
    public:
        ...
        bool Create(ObjectGuid::LowType guidlow, Map* map, uint32 phaseMask, uint32 entry, Position const& pos, CreatureData const* data = nullptr, bool dynamic = false);
        ...
        bool CanTrainingAndResetTalentsOf(Player* player) const;
    ...
    protected:
        bool CreateFromProto(ObjectGuid::LowType guidlow, uint32 entry, CreatureData const* data = nullptr);
