...
bool Creature::Create(ObjectGuid::LowType guidlow, Map* map, uint32 phaseMask, uint32 entry, Position const& pos, CreatureData const* data /*= nullptr*/, bool dynamic)
{
    ASSERT(map);
    SetMap(map);
    ...
    //! returning correct zone id for selecting OutdoorPvP script
    Relocate(pos);
...
bool Creature::CreateFromProto(ObjectGuid::LowType guidlow, uint32 entry, CreatureData const* data /*= nullptr*/)
{
    SetZoneScript();
...
bool Creature::LoadCreaturesAddon()
{
    ...
    {
        // 0 StandState
        // 1 Loyalty   Pet only, so always 0 for default creature
        // 2 StandFlags
        // 3 StandMiscFlags
