...
TempSummon* Transport::SummonPassenger(uint32 entry, Position const& pos, TempSummonType summonType, SummonPropertiesEntry const* properties /*= nullptr*/, uint32 duration /*= 0*/, Unit* summoner /*= nullptr*/, uint32 spellId /*= 0*/)
{
    Map* map = FindMap();
    if (!map)
        return nullptr;
    ...
    if (!summon->Create(map->GenerateLowGuid<HighGuid::Unit>(), map, phase, entry, { x, y, z, o }, nullptr))
    {
        delete summon;
        return nullptr;
    }
    ...
    else
    {
        ...
        for (PassengerSet::iterator itr = _passengers.begin(); itr != _passengers.end(); ++itr)
        {
            if ((*itr)->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                Transport::CalculatePassengerPosition(destX, destY, destZ, &destO, x, y, z, o);
