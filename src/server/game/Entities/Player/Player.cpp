...
void Player::SetDeathState(DeathState state)
{
    ...
    if (state == JUST_DIED && cur && ressSpellId)
...
void Player::LearnSpell(uint32 spell_id, bool dependent, uint32 fromSkill /*= 0*/)
{
    ...
    {
        WorldPacket data(SMSG_LEARNED_SPELL, 4 + 2);
...
float Player::GetRatingMultiplier(CombatRating cr) const
{
    ...
    if (GtCombatRatingsEntry const* Rating = sGtCombatRatingsStore.LookupEntry(cr * GT_MAX_LEVEL + level - 1))
...
void Player::UpdateZone(uint32 newZone, uint32 newArea)
{
    ...
    // check some item equip limitations
    AutoUnequipOffhandIfNeed();
...
void Player::_ApplyAllLevelScaleItemMods(bool apply)
{
    for (uint8 i = 0; i < INVENTORY_SLOT_BAG_END; ++i)
    {
        if (m_items[i])
        {
            ...
            _ApplyItemBonuses(proto, i, apply);
        }
    }
}
...
void Player::SendQuestGiverStatusMultiple()
{
    ...
    for (auto itr = m_clientGUIDs.begin(); itr != m_clientGUIDs.end(); ++itr)
    {
        ...
        if (itr->IsAnyTypeCreature())
        {
            ...
            Creature* questgiver = ObjectAccessor::GetCreatureOrPet(*this, *itr);
...
bool Player::LoadFromDB(ObjectGuid guid, SQLQueryHolder *holder)
{
    ...
    _LoadIntoDataField(fields[67].GetString(), PLAYER_FIELD_KNOWN_TITLES, KNOWN_TITLES_SIZE);
...
void Player::_LoadAuras(PreparedQueryResult result, uint32 timediff)
{
    ...
    QueryResult* result = CharacterDatabase.PQuery("SELECT casterGuid, spell, effectMask, recalculateMask, stackCount, amount0, amount1, amount2, base_amount0, base_amount1, base_amount2,
                                                        11          12          13              14
                                                    maxDuration, remainTime, remainCharges, applyResilience FROM character_aura WHERE guid = '%u'", GetGUID().GetCounter());
    */
...
void Player::_LoadGroup(PreparedQueryResult result)
{
    ...
    if (result)
    {
        if (Group* group = sGroupMgr->GetGroupByDbStoreId((*result)[0].GetUInt32()))
        {
            ...
            if (getLevel() >= LEVELREQUIREMENT_HEROIC) // the group leader may change the instance difficulty while the player is offline
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!IsGameMaster() && ar)
    {
        ...
        Difficulty target_difficulty = GetDifficulty();
...
void Player::SaveToDB(bool create /*=false*/)
{
    ...
    if (create)
    {
        ...
        stmt->setUInt8(index++, (uint8(GetDungeonDifficulty()) << 4));
...
void Player::SendResetInstanceFailed(uint32 reason, uint32 MapId) const
{
    ...
    */
    WorldPacket data(SMSG_INSTANCE_RESET_FAILED, 4 + 4);
    data << uint32(reason);
    data << uint32(MapId);
    SendDirectMessage(&data);
}
...
void Player::Whisper(uint32 textId, Player* target, bool /*isBossWhisper = false*/)
{
    ...
    WorldPacket data;
    ChatHandler::BuildChatPacket(data, CHAT_MSG_WHISPER, LANG_UNIVERSAL, this, target, bct->GetText(locale, getGender()), "", locale);
    target->SendDirectMessage(&data);
}
...
void Player::SendTaxiNodeStatusMultiple()
{
    for (auto itr = m_clientGUIDs.begin(); itr != m_clientGUIDs.end(); ++itr)
    {
        ...
        if (!nearestNode)
            continue;
        WorldPacket data(SMSG_TAXINODE_STATUS, 8 + 1);
        data << *itr;
...
bool Player::BuyItemFromVendorSlot(ObjectGuid vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    if (crItem->ExtendedCost)
    {
        ...
        if (GetMaxPersonalArenaRatingRequirement(0) < iece->reqpersonalarenarating)
        {
...
bool Player::SetHover(bool apply, bool packetOnly /*= false*/)
{
    ...
    WorldPacket data(apply ? SMSG_MOVE_SET_HOVER : SMSG_MOVE_UNSET_HOVER, 8 + 4);
    data << GetPackGUID();
    data << uint32(0);          //! movement counter
    SendDirectMessage(&data);
...
bool Player::SetWaterWalking(bool apply, bool packetOnly /*= false*/)
{
    ...
    WorldPacket data(apply ? SMSG_MOVE_WATER_WALK : SMSG_MOVE_LAND_WALK, 8 + 4);
    data << GetPackGUID();
    data << uint32(0);          //! movement counter
    SendDirectMessage(&data);
...
bool Player::SetFeatherFall(bool apply, bool packetOnly /*= false*/)
{
    ...
    WorldPacket data(apply ? SMSG_MOVE_FEATHER_FALL : SMSG_MOVE_NORMAL_FALL, 8 + 4);
    data << GetPackGUID();
    data << uint32(0);          //! movement counter
    SendDirectMessage(&data);
