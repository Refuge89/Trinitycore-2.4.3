...
bool WorldSocket::Update()
{
    ...
    while (_bufferQueue.Dequeue(queued))
    {
        ...
        header.cmd = queued->GetOpcode();
        EndianConvert(header.cmd);

        header.size = static_cast<uint16>(queued->size() + 2);
        EndianConvertReverse(header.size);

        if (queued->NeedsEncryption())
            _authCrypt.EncryptSend(reinterpret_cast<uint8*>(&header), sizeof(header));

        if (buffer.GetRemainingSpace() < queued->size() + sizeof(header))
        {
        ...
        if (buffer.GetRemainingSpace() >= queued->size() + sizeof(header))
        {
            buffer.Write(reinterpret_cast<uint8*>(&header), sizeof(header));
            if (!queued->empty())
                buffer.Write(queued->contents(), queued->size());
        }
        else    // single packet larger than 4096 bytes
        {
            MessageBuffer packetBuffer(queued->size() + sizeof(header));
            packetBuffer.Write(reinterpret_cast<uint8*>(&header), sizeof(header));
            if (!queued->empty())
