...
namespace Trinity
{
    ...
    {
        public:
            BroadcastTextBuilder(WorldObject const* obj, ChatMsg msgType, uint32 textId, uint8 gender, WorldObject const* target = nullptr)
                : _source(obj), _msgType(msgType), _textId(textId), _gender(gender), _target(target) { }
