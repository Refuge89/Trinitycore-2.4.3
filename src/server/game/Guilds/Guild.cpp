...
{
    WorldPacket data(SMSG_GUILD_INFO, m_name.size() + 4 + 4 + 4 + 4 + 4);
    data << m_name;

    tm createTm;
    localtime_r(&m_createdDate, &createTm);

    data << uint32(createTm.tm_mday);
    data << uint32(createTm.tm_mon + 1);
    data << uint32(createTm.tm_year + 1900);
