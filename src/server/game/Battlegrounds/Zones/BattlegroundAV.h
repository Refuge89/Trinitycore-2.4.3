...
class BattlegroundAV : public Battleground
{
    ...
    private:
        ...
        bool IsTower(BG_AV_Nodes node) { return _nodes[node].Tower; }
        ...
        /*variables */
        int32 _teamScores[2];
        uint32 _teamQuestStatus[2][9]; //[x][y] x=team y=questcounter
        BG_AV_NodeInfo _nodes[BG_AV_NODES_MAX];
        uint32 _mineOwner[2];
        uint32 _minePrevOwner[2]; //only for worldstates needed
        int32 _mineTimer; //ticks for both teams
        uint32 _mineReclaimTimer[2];
        uint32 _captainBuffTimer[2];
        bool _captainAlive[2];
        bool _isInformedNearVictory[2];
};
